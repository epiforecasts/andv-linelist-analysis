...
`Z[i]` (the offspring count for case `i`) is the number of secondaries
attributed to case `i` in the line list — see the `source_case` column in
`data/linelist.csv`. The model treats `Z[i]` as the observed outcome of a
Negative-Binomial draw with mean `R(t)` evaluated at `T_inf[i]`.

The `R(t)` reported by the model is the **case reproduction number** — the
expected total offspring of a case infected at time t, integrated over
their generation-interval distribution. Reports that present an
"instantaneous" R(t) (e.g., EpiEstim-style, indexed by transmission rather
than by source infection) lag the case `R(t)` by roughly the mean
generation interval.
...
All generated by `analyse()` from a fitted posterior.

### Reproduction number over time

Spaghetti of R(t) posterior draws over weekly bins. R = 1 dashed.

![R(t)](figures/Rt.png)

### Pair-level sense check

For each sourced pair, posterior median of `T_inf[secondary] − T_onset[source]`
(δ) overlaid on the fitted population Normal(μ_δ, σ_δ). The histogram is
visibly sharper than the fitted Normal — the within-day exposure encoding
inflates σ_δ relative to the across-pair spread.

![δ sense check](figures/delta_sense_check.png)

### Population-parameter pairplot

Posterior correlations and marginals for the population parameters.

![Pairplot](figures/pairplot.png)

### Prior predictives

Distributions of Inc, δ, GI/SI implied by the priors alone, before seeing
any data.

![Prior predictives](figures/prior_predictives.png)

### Posterior predictive vs observed

Predicted δ and SI from the fitted population distributions (blue) overlaid
with the observed per-pair posterior medians (orange). Discrepancies trace
back to the encoding-floor inflation of σ_δ; see Limitations.

![Posterior predictions](figures/posterior_predictions.png)
