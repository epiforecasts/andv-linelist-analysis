...
## - `main(args)` is the CLI entry point invoked by
##   `julia -e 'using TransmissionLinelist; main(ARGS)'` (a plain function, not
##   `@main`, so `using` never auto-runs `analyse()` — see #43).
