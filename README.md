...
derived from the fitted distributions in post-processing and cross-checked
against per-pair latent event times as a model diagnostic.
...
Sources and secondaries are exchangeable in the population model so the two
share the same marginal mean and SD analytically.
Empirical estimates are recovered directly from per-pair latent event times
(`T_inf`, `T_onset`); `compare_intervals(post, pairs)` prints the comparison.
...
Per case the mean is `R(t)` evaluated at the case's infection time, and `k`
is the dispersion.
...
figures/
  ...
  pairplot.png              — posterior pairplot of population parameters (generated)
  prior_predictives.png     — prior predictive distributions (generated)
  posterior_predictions.png — posterior predictive check for δ and SI (generated)
  gi_si_comparison.png      — empirical GI vs SI vs analytical (generated)
  incubation_check.png      — empirical incubation vs fitted LogNormal (generated)
