...
# `build_data` re-encodes exposure / onset windows as day offsets from `t0` (60 days before the first onset), and `bin_edges_day` builds the weekly R(t) knot dates from the same origin. Passing those into `joint_model` instantiates the Turing model directly.
